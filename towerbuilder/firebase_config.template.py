...
# IMPORTANT: Do NOT commit firebase_config.py to version control
...
FIREBASE_CONFIG = {
    ...
    # NOTE: Realtime Database URL - sử dụng URL theo region của bạn
    # Ví dụ: https://YOUR_PROJECT-default-rtdb.asia-southeast1.firebasedatabase.app
...
class FirebaseAuth:
    ...
    def save_score(self, score, level, mode="infinite"):
        """
        Lưu điểm lên Firebase theo mode (infinite/timed/...)
    ...
    def get_leaderboard(self, limit=10, mode="infinite"):
        ...
        Args:
            limit: Số lượng người chơi tối đa (default: 10)
            mode: Chế độ game - "infinite" hoặc "timed" (default: "infinite")
        
        Returns:
            List[dict]: Danh sách người chơi với username, score, level
        """
        if not self.is_logged_in():
        ...
        try:
            ...
            # Đọc toàn bộ /scores — được phép nhờ Firebase rules
