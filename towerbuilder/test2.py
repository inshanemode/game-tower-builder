...
class Block(pygame.sprite.Sprite):
    ...
    def drop(self, tower, current_level=1, wind_force=0):
        if self.state == "swinging":
            ...
            # Tính vận tốc ban đầu từ chuyển động con lắc
            # Vận tốc ngang = tốc độ góc * bán kính * cos(góc)
            self.velocity_x = self.speed * rope_length * 15  # Nhân 15 để tăng hiệu ứng
            # Vận tốc dọc = tốc độ góc * bán kính * sin(góc) 
            self.velocity_y = abs(self.speed * rope_length * 8)  # Nhân 8 cho phần dọc
        ...
        if self.state == "falling":
            # Chuyển động parabol với vận tốc ban đầu từ con lắc
            self.velocity_y += grav  # Trọng lực tác động lên vận tốc Y
            self.y += self.velocity_y
            
            # Di chuyển ngang với vận tốc ban đầu + gió
            if hasattr(self, 'wind_drift'):
                self.wind_drift += wind_force * 0.3
                self.xlast += self.velocity_x + self.wind_drift
                self.wind_drift *= 0.95
                # Giảm dần vận tốc ngang (ma sát không khí)
